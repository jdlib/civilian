...
public class TempContextTest extends CivTest
{
	@Test public void test() throws Throwable
	{
		...
		for (Method method : context.getClass().getDeclaredMethods())
		{
			if (!Modifier.isPrivate(method.getModifiers()) && !method.getName().contains("cobertura"))
			{
				try
				{
					method.invoke(context, getDummyMethodParams(method));
				}
				catch(InvocationTargetException e)
				{
					...
					if (cause == null)
						throw e;
					if ((cause.getMessage() == null) || !cause.getMessage().equals("Context method may not be called before Application#init was called"))
