...
import org.civilian.samples.crm.db.SearchResult;
...
// maps to /contacts/search
public class SearchController extends ContactsController
{
	...
	// maps to /contacts/search/filter
	@Segment("filter")
	@Consumes(ContentType.Strings.APPLICATION_JSON) 
	@Get public void getFilters(NgReply reply) throws Exception
	{
		KeyListBuilder<String> builder = new KeyListBuilder<>();
		builder.add(ContactService.FILTER_NAME, 		msg(Message.Name));
		builder.add(ContactService.FILTER_FIRSTNAME,	msg(Message.FirstName));
		builder.add(ContactService.FILTER_CUSTOMER,		msg(Message.Customer));
		builder.add(ContactService.FILTER_PHONE,		msg(Message.Phone));
