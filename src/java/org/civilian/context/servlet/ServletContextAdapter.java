...
/**
 * A Context implementation which wraps a javax.servlet.ServletContext.
 */
class ServletContextAdapter extends Context
{
	/**
	...
	/**
	 * Returns the ServletContextAdapter instance associated with the ServletContext.
	 */
	public static ServletContextAdapter getInstance(ServletContext context)
	{
		return (ServletContextAdapter)context.getAttribute(ServletContextAdapter.class.getName());
	}
	...
	/**
	 * Creates a ServletContextAdapter.
	 */
	public ServletContextAdapter(ServletContext servletContext)
	{
		...
		{
			Logs.CONTEXT.error("could not initialize", e);
			close();
		}
	}
	...
	{
		AppServlet servlet = app.getConnector(AppServlet.class);
		if (servlet != null)
			servlet.close(); 
	}
	...
	//-----------------------------------------------------------------
	// implementation of Context API using the ServletContext
	//-----------------------------------------------------------------
